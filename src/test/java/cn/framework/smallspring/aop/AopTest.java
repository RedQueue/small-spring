...
import cn.framework.smallspring.aop.aspectj.AspectJExpressionPointcut;
...
public class AopTest {
    ...
    public void test_proxy_method() {
        ...
        IUserService proxy = (IUserService) Proxy.newProxyInstance(Thread.currentThread().getContextClassLoader(), targetObj.getClass().getInterfaces(), new InvocationHandler() {
            // 方法匹配器
            MethodMatcher methodMatcher = new AspectJExpressionPointcut("execution(* cn.framework.smallspring.aop.IUserService.*(..))");
    ...
    public void test_dynamic() {
        // 目标对象
        IUserService userService12 = new UserService12();

        // 组装代理信息
        AdvisedSupport advisedSupport = new AdvisedSupport();
        advisedSupport.setTargetSource(new TargetSource(userService12));
        advisedSupport.setMethodInterceptor(new UserService12Interceptor());
        advisedSupport.setMethodMatcher(new AspectJExpressionPointcut("execution(* cn.framework.smallspring.aop.IUserService.*(..))"));
